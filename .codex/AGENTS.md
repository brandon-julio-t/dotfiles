...
- Omit the subagent model override by default so the subagent uses the current model. Set model and reasoning-effort overrides only through subagent tool parameters, and only when explicitly requested or when a task-specific reason justifies them.
