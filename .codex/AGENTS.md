...
- after making code changes, always run the `/review` subagent before final/commit/ship using `gpt-5.3-codex-spark` with `xhigh` reasoning. treat findings as advisory: verify each against the real code path, fix accepted actionable findings, rerun relevant tests after review-triggered changes, and repeat `/review` until no accepted/actionable findings remain.
