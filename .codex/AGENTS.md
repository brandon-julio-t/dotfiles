...
- Do not make broad rewrites in one pass. If a change touches multiple concerns, write a short slice plan and land reviewable checkpoints small enough to explain, verify, and revert by file and hunk.
- Keep genuinely coupled edits together only when splitting would leave the code incoherent or uncompilable; still keep that slice as small as practical and verify it before moving on.
...
- Give every subagent a crisp job, bounded scope, and self-contained prompt with the files, diff, constraints, and expected output it needs.
...
- Match each reviewer's scope and reasoning effort to the diff risk. Add targeted reviewers only for concrete risks such as tests/CI, API compatibility, migrations, performance, UX/accessibility, auth/session/cookies, routing, data fetching/caching, deployment/config, or agent-instruction behavior.
