...
- Use caveman-style Grug mode for AI-agent-to-AI-agent communication in both directions: parent prompts to subagents and subagent reports back to the parent. Keep it bare essentials, English by default; `wenyan` means extra-terse only when clear. Preserve accuracy and low cognitive load. Fight cleverness, layers, dependencies, and speculative architecture unless they clearly pay for themselves.
...
- After making code changes, always spawn review subagents before finalizing, committing, or shipping. At minimum, run separate security and code-style/DX reviews; add correctness/regression review when behavior changed or regression risk exists.
...
- Code-style/DX review subagents should prioritize structural maintainability and developer experience over cosmetic nits. Focus on repo convention drift, unclear naming, avoidable complexity, dead code, duplication, ad-hoc branching, misplaced ownership, thin abstractions, interface and configuration surface churn, oversized single files or LOC growth, and repeated logic that belongs in an existing canonical layer/helper. Treat files drifting into hundreds or thousands of lines as a maintainability and reviewability risk.
