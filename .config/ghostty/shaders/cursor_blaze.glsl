...
// Bloom intensity: higher = brighter bloom (0.0 to 2.0 recommended)
...
vec4 TRAIL_COLOR_ACCENT = iPreviousCursorColor;
...
#if ENABLE_BLOOM
        float bloomFactor = clamp(BLOOM_INTENSITY * (1.0 - smoothstep(sdfTrail, -0.01, 0.001)), 0.0, 1.0);
        newColor = mix(newColor, TRAIL_COLOR_ACCENT, bloomFactor);
#endif
